...
psiQGrid = cell(NfourierIn-1,1);            % First phase is zero by convention.
...
SONICInitM = zeros(numel(QmRange),numel(USPaRange),numel(USfreqRange),numel(aBLSRange),numel(fBLSRange),numel(DeltaQmRange)^NfourierIn,numel(psiQRange)^(NfourierIn-1));
SONICInitC = cell(numel(QmRange),numel(USPaRange),numel(USfreqRange),numel(aBLSRange),numel(fBLSRange),numel(DeltaQmRange)^NfourierIn,numel(psiQRange)^(NfourierIn-1));
