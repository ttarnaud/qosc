function Out = SONIC_GPi_nanoMC(ESi,USPaT,DISPLAY,tNICE,t,Q1,h1,n1,r1,CA1,Q2,h2,n2,r2,CA2,Gna,Vna,Gk,Vk,Gl,Vl,...
    GT,VT,GCa,VCa,Gahp,minf,ainf,sinf,f1Veff0,f1VeffPa,f1rt0,f1rtPa,f1rtV,SONICgates,Cm0,aBLS,fBLS,RSI,proteinMode,gateMultip)
if DISPLAY == 1
...
Out1 = [ESi(t)+10^(-3)*(1/(pi*aBLS^2*RSI))*(Veff2-Veff1)-10^(-3)*(xl*Gl*(Veff1-Vl)+xP*Gna*minf(Veff1).^3.*h1.*(Veff1-Vna)+...
   xP*Gk*n1.^4.*(Veff1-Vk)+xP*GT*ainf(Veff1).^3.*r1.*(Veff1-VT)+...
   xP*GCa.*sinf(Veff1).^2.*(Veff1-VCa)+xP*Gahp*(Veff1-Vk).*(CA1./(CA1+10)));
cellfun(@(X) f1rt0.(['a_' X])(Q1)-f1rt0.(['apb_' X])(Q1)*rate1.(X),SONICgates)
-10^(-4)*(0.1*GT*ainf(Veff1).^3.*r1.*(Veff1-VT)+0.1*GCa.*sinf(Veff1).^2.*(Veff1-VCa)+15*1000*CA1)];
Out2 = [ESi(t)+10^(-3)*(1/(pi*aBLS^2*(1/fBLS-1)*RSI))*(Veff1-Veff2)-10^(-3)*(Ml*Gl*(Veff2-Vl)+MP*Gna*minf(Veff2).^3.*h2.*(Veff2-Vna)+...
   MP*Gk*n2.^4.*(Veff2-Vk)+MP*GT*ainf(Veff2).^3.*r2.*(Veff2-VT)+...
   MP*GCa.*sinf(Veff2).^2.*(Veff2-VCa)+MP*Gahp*(Veff2-Vk).*(CA2./(CA2+10)));
cellfun(@(X) f1rtV.(['a_' X])(Veff2)-f1rtV.(['apb_' X])(Veff2)*rate2.(X),SONICgates)
-10^(-4)*(0.1*GT*ainf(Veff2).^3.*r2.*(Veff2-VT)+0.1*GCa.*sinf(Veff2).^2.*(Veff2-VCa)+15*1000*CA2)];
