function Out = SONIC_RSFS_nanoMC(ESi,USPaT,DISPLAY,tNICE,t,Q1,h1,m1,n1,p1,Q2,h2,m2,n2,p2,Gna,Vna,Gk,Vk,Gm,Gl,Vl,f1Veff0,f1VeffPa,f1rt0,f1rtPa,f1rtV,SONICgates,Cm0,aBLS,fBLS,RSI,proteinMode,gateMultip)
if DISPLAY == 1
...
Out1 = [ESi(t)+10^(-3)*(1/(pi*aBLS^2*RSI))*(Veff2-Veff1)-10^(-3)*(xP*Gna*m1^3*h1*(Veff1-Vna)+xP*Gk*n1^4*(Veff1-Vk)+xP*Gm*p1*(Veff1-Vk)+xl*Gl*(Veff1-Vl));
cellfun(@(X) f1rt0.(['a_' X])(Q1)-f1rt0.(['apb_' X])(Q1)*rate1.(X),SONICgates)];
Out2 = [ESi(t)+10^(-3)*(1/(pi*aBLS^2*(1/fBLS-1)*RSI))*(Veff1-Veff2)-10^(-3)*(MP*Gna*m2^3*h2*(Veff2-Vna)+MP*Gk*n2^4*(Veff2-Vk)+MP*Gm*p2*(Veff2-Vk)+Ml*Gl*(Veff2-Vl));
